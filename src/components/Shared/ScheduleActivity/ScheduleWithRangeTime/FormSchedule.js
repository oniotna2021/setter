...
import { formatDateToHHMMSS, convertH2M } from 'utils/misc';
...
const FormSchedule = ({ duration, assingSameHour, selectedDays, currentDayId, schedules, setSchedules }) => {
        ...
        if(assingSameHour) {
            if(duration !== null && isDate(duration)) {
                ...
                setSchedules(schedules => schedules.map((schedule) => {
                    if(selectedDays.some((day) => schedule.day_week_id === day)) {
                        return {
                            ...schedule,
                            [nameTime]: hourFormat,
