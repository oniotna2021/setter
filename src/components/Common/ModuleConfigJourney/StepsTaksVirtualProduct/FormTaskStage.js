...
import {
  ...
  putTaskSteps,
  deleteTaskSteps,
} from "services/JourneyModule/TaskJourney";
...
const FormTaskStage = ({
  ...
  const onSubmit = (value) => {
    ...
    const functionCall = type === "Nuevo" ? postTaskSteps : putTaskSteps;

    functionCall(type === "Nuevo" ? value : defaultValue?.uuid, value)
      .then(({ data }) => {
    ...
    Swal.fire({
      ...
      .then((result) => {
        if (result.isConfirmed) {
          deleteTaskSteps(defaultValue?.uuid)
            .then((req) => {
              Swal.fire(
                "¡Eliminado!",
