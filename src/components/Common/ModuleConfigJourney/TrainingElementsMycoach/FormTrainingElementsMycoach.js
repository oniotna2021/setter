...
import {
  ...
  putTrainingElements,
  deleteCardiacDiseases,
} from "services/JourneyModule/TrainingElementsMycoach";

// utils
import { successToast, errorToast, mapErrors } from "utils/misc";

const FormTrainingElementsMycoach = ({
  permissionsActions,
  defaultValue,
  setLoad,
  load,
    ...
    Swal.fire({
      ...
      .then((result) => {
        if (result.isConfirmed) {
          deleteCardiacDiseases(defaultValue.id)
            .then((req) => {
              Swal.fire(
                "¡Eliminado!",
...
export default FormTrainingElementsMycoach;
