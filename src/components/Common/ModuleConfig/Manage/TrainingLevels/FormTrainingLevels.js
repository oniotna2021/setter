...
import { TrainingLevelsForm } from "config/Forms/ConfigForms";
...
export const FormTrainingLevels = ({
  type,
  defaultValue,
  setExpanded,
  load,
  ...
  const onSubmit = (value) => {
    ...
    const functionCall =
      type === "Nuevo" ? postTrainingLevels : putTrainingLevels;
    ...
    Swal.fire({
      ...
      .then((result) => {
        if (result.isConfirmed) {
          deleteTrainingLevels(defaultValue.id)
            .then((req) => {
              Swal.fire(
                "¡Eliminado!",
