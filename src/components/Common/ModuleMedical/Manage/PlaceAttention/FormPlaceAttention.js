...
import { PlaceAttentionForm } from "config/Forms/MedicalForms";
...
import {
  postPlaceAttention,
  putPlaceAttention,
  deletePlaceAttention,
} from "services/MedicalSoftware/PlaceAttention";
...
export const FormPlaceAttention = ({
  type,
  defaultValue,
  setExpanded,
  load,
  ...
  const onSubmit = (value) => {
    ...
    const functionCall =
      type === "Nuevo" ? postPlaceAttention : putPlaceAttention;
  ...
  const deleteForm = () => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        deletePlaceAttention(defaultValue.id)
          .then((req) => {
            Swal.fire(
