...
import { SystemGatewayForm } from "config/Forms/MedicalForms";
...
import {
  ...
  putSystemGateway,
  deleteSystemGateway,
} from "services/MedicalSoftware/SystemGateway";
...
export const FormSystemGateway = ({
  type,
  defaultValue,
  setExpanded,
  load,
  ...
  const onSubmit = (value) => {
    ...
    const functionCall =
      type === "Nuevo" ? postSystemGateway : putSystemGateway;
  ...
  const deleteForm = () => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        deleteSystemGateway(defaultValue.id)
          .then((req) => {
            Swal.fire(
