...
import { QueryTypeForm } from "config/Forms/MedicalForms";
...
import {
  ...
  putQueryType,
  deleteQueryType,
} from "services/MedicalSoftware/QueryType";
...
export const FormQueryType = ({
  ...
  const [loadingFetch, setLoafingFetch] = useState(false);
  ...
  const onSubmit = (value) => {
    ...
    setLoafingFetch(true);
    const functionCall = type === "Nuevo" ? postQueryType : putQueryType;
  ...
  const deleteForm = () => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        deleteQueryType(defaultValue.id)
          .then((req) => {
            Swal.fire(
