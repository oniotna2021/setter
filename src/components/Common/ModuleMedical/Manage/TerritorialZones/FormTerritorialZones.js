...
import { TerritorialZonesForm } from "config/Forms/MedicalForms";
...
//Services
import {
  postTerritorialZones,
  putTerritorialZones,
  deleteTerritorialZones,
} from "services/MedicalSoftware/TerritorialZones";
...
export const FormTerritorialZones = ({
  type,
  defaultValue,
  setExpanded,
  load,
  ...
  const onSubmit = (value) => {
    ...
    const functionCall =
      type === "Nuevo" ? postTerritorialZones : putTerritorialZones;
  ...
  const deleteForm = () => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        deleteTerritorialZones(defaultValue.id)
          .then((req) => {
            Swal.fire(
