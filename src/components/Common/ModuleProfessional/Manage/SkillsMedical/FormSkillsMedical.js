...
import { SkillsMedicalForm } from "config/Forms/ProfessionalForms";
...
import {
  ...
  putSkillsMedical,
  deleteSkillsMedical,
} from "services/Professional/Skills";
...
export const FormSkillsMedical = ({ type, defaultValue, setExpanded }) => {
  ...
  const onSubmit = (value) => {
    const functionCall =
      type === "Nuevo" ? postSkillsMedical : putSkillsMedical;
  ...
  const deleteForm = () => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        deleteSkillsMedical(defaultValue.id)
          .then((req) => {
            Swal.fire(
