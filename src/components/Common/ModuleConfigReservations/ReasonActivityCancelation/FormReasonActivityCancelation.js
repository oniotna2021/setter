...
import {
  postActivityReason,
  putActivityReason,
  deleteActivityReason,
} from "services/Reservations/activitiesCancelationReason";
...
export const FormReasonActivityCancelation = ({
  type,
  defaultValue,
  setExpanded,
  load,
  ...
  const onSubmit = (value) => {
    ...
    const functionCall =
      type === "Nuevo" ? postActivityReason : putActivityReason;
  ...
  const deleteForm = () => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        deleteActivityReason(defaultValue?.id)
          .then((req) => {
            Swal.fire(
